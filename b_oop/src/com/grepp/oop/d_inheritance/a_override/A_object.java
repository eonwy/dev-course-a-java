...
public class A_object {
    ...
    // NOTE DA02 : override
    // override : 재정의, 부모클래스로부터 상속받은 메서드를 자식클래스에서 재정의 하는 것
    //            반환타입, 매개변수, 이름이 일치해야한다.
    //            접근제한자는 더 넓은 범위로만 재정의할 수 있다.
    //            protected -> public (0) public -> protected (x)
    // NOTE DA03 : annotation
    // annotation : 주석 (코드에 대한 정보), 메타데이터(데이터에 대한 데이터)
    // 컴파일되거나 실행되는 시점에 컴파일러 및 외부 모듈에게 필요한 정보를 전달
    // @Override : 이 메서드는 부모클래스로부터 상속받은 메서드를 override(재정의) 한 메서드. 컴파일러에게 전달
    //             컴파일러는 부모클래스에 이 메서드와 반환타입, 매개변수, 이름이 일치하는 메서드가 있는지 확인 후 없으면 에러
    @Override
    ...
    // hash : 어떤 데이터를 일정한 크기(bit 수)의 값을 변환하는 것
    // 해시함수의 규칙
    // 1. 같은 값을 넣으면 언제나 같은 해시를 반환
    // 2. 다른 값을 넣으면 언제나 다른 해시를 반환
    // ex) SHA-256 : 어떤 데이터를 256bit 의 데이터로 변환하는 함수

    // 같은 객체라면 언제나 같은 해시를 가져야하기 때문에
    // 객체의 같음을 판단하는 equals 가 override 되었다면 hashCode 도 함께 override 되어야 한다
    @Override
    public int hashCode() {
