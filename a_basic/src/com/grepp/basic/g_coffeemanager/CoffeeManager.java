...
public class CoffeeManager {
    ...
    private static void app() {
        ...
        Coffee juice = new Coffee("juice", 4000, 2000, 10, 3, 0);

        Coffee[] coffees = {americano, mocha, latte, juice};
        ...
        while (true) {
            ...
            if (menu == 1) {
                ...
                Coffee item = coffees[code];

                if (orderCnt <= item.stock) {
                    // 판매
                    // 재고 반영
                    item.stock -= orderCnt;
                    // 판매량 반영
                    item.salesCnt += orderCnt;
                    // 잔고 반영
                    balance += item.price * orderCnt;
                    // 매출 반영
                    salesAmount += item.price * orderCnt;
                } else {
                    // 매입
                    // 매입금액 산정
                    int purchasePrice = item.cost * orderCnt;
                    ...
                    // 매입 + 판매
                    System.out.println("system: " + item.name + " 재고 " + orderCnt + "개 확보합니다.");
                    // 매입
                    // 재고 반영
                    item.stock += orderCnt;
                    // 잔고 반영
                ...
                if (item.stock <= item.safetyStock) {
                    // 매입금액 산정
                    int purchasePrice = item.safetyStock * 2 * item.cost;
                    if (balance >= purchasePrice) {
                        // 매입
                        // 재고 반영
                        item.stock += item.safetyStock * 2;
                        // 잔고 반영
                        balance -= purchasePrice;
                        // 지출 반영
                        expensesAmount += purchasePrice;
                ...
                System.out.println("\n 제품명 : " + item.name
                    + " \n 판매가 : " + item.price
                    + " \n 판매수량 : " + orderCnt
                    + " \n 결재금액 : " + item.price * orderCnt
                    + " \n 남은 재고 : " + item.stock
                );
            } else {
                for (Coffee item : coffees) {
                    System.out.printf("%-10s 재고(%3d) 판매량(%3d) \n", item.name, item.stock, item.salesCnt);
                }
